...
function foldOrphanLines(block) {
    ...
    for (const line of lines) {
        ...
        if (!trimmed)
            continue;
        const isOrphan = trimmed.length === 1 && /[\u4e00-\u9fffA-Za-z0-9]/u.test(trimmed);
...
export function normalizeEditableText(editable) {
    return editable
        .split(/\n\n+/)
...
export function applyEditableToHtml(html, editable) {
    ...
    let blockIdx = 0;
    const marked = html.replace(/<br\s*\/?>/gi, BR);
    const merged = marked.replace(/>([^<]+)</g, (full, inner) => {
        const normalized = inner.replace(new RegExp(BR, 'g'), '\n').trim();
        if (!normalized || isSkippableText(normalized))
            return full;
        ...
        const newInner = hasBr && lines.length > 1
