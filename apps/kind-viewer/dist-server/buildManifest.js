...
function extractTitle(html) {
    ...
    if (section)
        return stripTags(section[1]);
    const t = html.match(/<title>([^<]*)<\/title>/i);
    if (t)
        return stripTags(t[1]);
    return 'Untitled';
}
function extractBriefing(html) {
    const mission = [...html.matchAll(/class="[^"]*mission-text[^"]*"[^>]*>([\s\S]*?)<\//gi)]
        .map((m) => stripTags(m[1] ?? ''))
        .filter(Boolean);
    if (mission.length)
        return mission.join(' · ').slice(0, 240);
    const belief = html.match(/class="[^"]*belief-text[^"]*"[^>]*>([\s\S]*?)<\//i);
    if (belief)
        return stripTags(belief[1]).slice(0, 240);
    const tagline = html.match(/class="[^"]*tagline[^"]*"[^>]*>([\s\S]*?)<\//i);
    if (tagline)
        return stripTags(tagline[1]).slice(0, 240);
...
async function loadSlideState(deckRoot) {
    try {
...
async function loadZhMap(deckRoot) {
    try {
...
export async function buildManifest(deckRoot) {
...
export function safeSlidePath(deckRoot, rel) {
