...
function classify(label) {
    ...
    if (/left-panel|huge-text|huge-title|vision-big|subtitle/.test(l))
        return 'left';
    if (/right-panel|problem-grid|summary-box/.test(l))
        return 'right';
    if (/header-section|tagline/.test(l))
        return 'header';
    if (/problem-row|mission-item|summary-item|talent-row|risk-row|allocation|use-case|product-card|stage|moat|founding|team-member|ecosystem|financing|pillar|benefit-item|outcome-item|mentorship-item|content-block/.test(l))
        return 'row';
    if (/deco/.test(l))
        return 'deco';
    return 'other';
...
export function optimizeLayoutRules(blocks, labels, _lang) {
    ...
    const other = meta.filter((b) => !['deco', 'left', 'right', 'header', 'row'].includes(classify(b.label)));
    for (const b of deco) {
    ...
    for (const id of ids) {
        if (!result[id])
            result[id] = clampBox(blocks[id]);
    }
    return result;
}
function readAiConfig() {
    const apiKey = process.env.KIND_LAYOUT_AI_API_KEY?.trim() || process.env.OPENAI_API_KEY?.trim() || '';
    if (!apiKey)
        return null;
    const baseUrl = (process.env.KIND_LAYOUT_AI_BASE_URL?.trim() ||
        process.env.OPENAI_BASE_URL?.trim() ||
        'https://api.openai.com/v1').replace(/\/$/, '');
    const model = process.env.KIND_LAYOUT_AI_MODEL?.trim() || process.env.OPENAI_MODEL?.trim() || 'gpt-4o-mini';
    return { apiKey, baseUrl, model };
}
async function optimizeLayoutWithAi(blocks, labels, lang, slideHint, config) {
