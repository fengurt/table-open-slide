...
export function findUserByEmail(email) {
    ...
    const row = getDb()
        .prepare(`SELECT email, secret_hash, role, created_at, updated_at
...
export function createUser(email, secret, role = 'viewer') {
    const normalized = email.trim().toLowerCase();
    if (!normalized)
...
export function verifyUserCredentials(email, secret) {
