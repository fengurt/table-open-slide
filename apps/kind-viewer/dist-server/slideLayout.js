...
function layoutPath(deckRoot, rel) {
    return path.join(deckRoot, 'layouts', `${slideIdFromRel(rel)}.json`);
}
export async function readSlideLayoutFile(deckRoot, rel) {
    try {
...
export async function writeSlideLayout(deckRoot, rel, lang, blocks, labels) {
    const dir = path.join(deckRoot, 'layouts');
    await fs.mkdir(dir, { recursive: true });
    const existing = (await readSlideLayoutFile(deckRoot, rel)) ?? { version: 1 };
    const next = {
        version: 1,
...
function isPanelLabel(label) {
    return label === 'left-panel' || label === 'right-panel';
}
export function buildLayoutCss(blocks, labels = {}) {
    ...
    for (const [id, b] of Object.entries(blocks)) {
        ...
        if (isPanelLabel(label)) {
            hasPanel = true;
            if (label === 'left-panel') {
                rules.push(`[data-kind-block="${id}"]{width:${b.w}%!important;flex:0 0 auto!important;position:relative!important;height:100%!important;left:auto!important;top:auto!important;margin:0!important;}`);
            }
            else {
                rules.push(`[data-kind-block="${id}"]{flex:1 1 0!important;position:relative!important;height:100%!important;left:auto!important;top:auto!important;min-width:0!important;margin:0!important;}`);
            }
        }
        else {
            rules.push(`[data-kind-block="${id}"]{left:${b.x}%;top:${b.y}%;width:${b.w}%;height:${b.h}%;}`);
        }
    }
...
`;
export function injectLayoutProbeScript() {
...
export async function getLayoutPayload(deckRoot, rel, lang, labelsFromHtml) {
