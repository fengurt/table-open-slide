import {
  ...
  primaryClassFromLabel,
} from './slideLayoutObjects.js';

const DECO_ONLY = /^deco-/;

function extractSlideContainerInner(
  html: string,
...
function divDepthBefore(inner: string, offset: number): number {
...
function nextId(labels: Record<string, string>): string {
  let n = 0;
...
function isInsidePanel(html: string, offset: number): boolean {
...
function applyInserts(html: string, inserts: InsertOp[]): string {
...
function tagLayoutObjects(inner: string, labels: Record<string, string>): string {
  ...
  const itemClasses = LAYOUT_OBJECT_CLASSES.filter(
    (c) => !(LAYOUT_PANEL_CLASSES as readonly string[]).includes(c),
  );
...
function tagAllInDocument(html: string, labels: Record<string, string>): string {
  ...
  for (const cls of LAYOUT_OBJECT_CLASSES) {
    ...
    out = out.replace(re, (match, attrs: string) => {
      if (/data-kind-block=/i.test(attrs)) return match;
      const id = nextId(labels);
      labels[id] = cls;
...
export function tagSlideBlocks(html: string): { html: string; labels: Record<string, string> } {
...
export function blockClassFromLabel(label: string): boolean {
  return !DECO_ONLY.test(label);
}

export function stripKindBlockAttrs(html: string): string {
