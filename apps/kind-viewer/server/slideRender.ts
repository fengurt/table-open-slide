...
import {
  buildLayoutCss,
  injectLayoutProbeScript,
  layoutForLang,
  readSlideLayoutFile,
  TYPOGRAPHY_CSS,
} from './slideLayout.js';

export type SlideLang = 'en' | 'zh';
...
const FONT_LINK =
  '<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700;800&family=Work+Sans:wght@400;600&display=swap" rel="stylesheet">';
...
let deckBaseCssCache: string | null = null;

async function getDeckBaseCss(deckRoot: string): Promise<string> {
  if (deckBaseCssCache !== null) return deckBaseCssCache;
  try {
...
const HOST_LAYOUT_ROW =
  '.slide-md-host .slide-container{display:flex!important;flex-direction:row!important;height:720px;min-height:720px;max-height:720px;width:1280px}';
const HOST_LAYOUT_STACK =
  '.slide-md-host .slide-container{display:flex!important;flex-direction:column!important;height:720px;min-height:720px;max-height:720px;width:1280px}';

/** Only inspect `.slide-container { ... }`, not child rules like `.right-panel { flex-direction: column }`. */
function slideContainerUsesColumnLayout(styles: string): boolean {
  for (const m of styles.matchAll(/\.slide-container\s*\{([^}]*)\}/gi)) {
    if (/flex-direction:\s*column/i.test(m[1])) return true;
  }
  return false;
}
...
export function splitLocaleMd(raw: string): Record<SlideLang, string> {
...
function extractStylesFromHtml(html: string): string {
  ...
  if (blocks.length === 0) return DEFAULT_STYLES;
...
function injectViewerStyles(html: string): string {
  if (html.includes('kind-viewer-stage')) return html;
  const tag = `<style>${VIEWER_STYLES}</style>`;
  if (/<\/head>/i.test(html)) return html.replace(/<\/head>/i, `${tag}\n</head>`);
  return `${tag}\n${html}`;
}

function wrapInSlideShell(
  bodyInner: string,
  styles: string,
  lang: SlideLang,
  embedRoot: boolean,
...
async function applySlidePostProcess(
  html: string,
  deckRoot: string,
  rel: string,
  lang: SlideLang,
  options: { layoutProbe?: boolean; layoutApply?: boolean; layoutEdit?: boolean },
): Promise<string> {
  ...
  let out = tagged.html;
  const layoutFile =
    layoutApply || layoutEdit ? await readSlideLayoutFile(deckRoot, rel) : null;
...
export async function resolveSlideHtml(
  deckRoot: string,
  rel: string,
  lang: SlideLang,
  options?: { layoutProbe?: boolean; layoutApply?: boolean; layoutEdit?: boolean },
