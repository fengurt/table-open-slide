...
function findBlockOpen(html: string, blockId: string): { tag: string; start: number; openEnd: number } | null {
  const id = blockId.replace(/"/g, '');
  const re = new RegExp(
    `<(div|h[12])(\\s[^>]*data-kind-block="${id}"[^>]*)>`,
    'i',
  );
...
function findMatchingClose(html: string, tag: string, openEnd: number): number | null {
  if (tag === 'div') {
    ...
    while (i < slice.length && depth > 0) {
      ...
      if (isOpen) {
        ...
        depth -= 1;
        if (depth === 0) return openEnd + nextClose + 6;
        i = nextClose + 6;
      }
    }
    return null;
...
export function extractBlockFragment(html: string, blockId: string): string | null {
...
export function blockFragmentToEditable(fragment: string): string {
  return htmlToEditable(fragment);
}

export function applyEditableToBlockFragment(fragment: string, editable: string): string {
  return applyEditableToHtml(fragment, editable);
}

export function applyBlockEditableToSlideHtml(
  slideHtml: string,
  blockId: string,
  editable: string,
): string | null {
...
export function patchBlockFieldInFragment(
  fragment: string,
  fieldClass: string,
  plainText: string,
): string | null {
  if (!FIELD_CLASS_RE.test(fieldClass)) return null;
  const re = new RegExp(
    `<([a-z][a-z0-9]*)(\\s[^>]*class="[^"]*\\b${fieldClass}\\b[^"]*"[^>]*)>([\\s\\S]*?)</\\1>`,
    'i',
  );
...
export function patchBlockFieldInSlideHtml(
  slideHtml: string,
  blockId: string,
  fieldClass: string,
  plainText: string,
