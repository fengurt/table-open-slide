...
): { email: string; role: UserRole } | null {
  getDb();
...
export async function handleApi(
  ...
  if (pathname === '/api/auth/login' && method === 'POST') {
    getDb();
    try {
      const body = (await readJsonBody(req)) as { email?: string; secret?: string };
  ...
  if (pathname === '/api/slide-md' && method === 'PUT') {
    ...
    try {
      const body = (await readJsonBody(req)) as { editable?: string; lang?: string };
    ...
    try {
      ...
      const ok =
        typeof body.field === 'string' && body.field.length > 0
          ...
          : await writeSlideBlockEditable(
              ctx.deckRoot,
              rel,
              lang,
              blockId,
              String(body.editable ?? ''),
            );
      if (!ok) {
        sendJson(res, 404, { error: 'block not found' });
        return true;
  ...
  if (pathname === '/api/slide-layout' && method === 'GET') {
    ...
    try {
      ...
      const labelsFromHtml: Record<string, string> = {};
  ...
  if (pathname === '/api/slide-layout/optimize' && method === 'POST') {
    ...
    try {
      ...
      const payload = await getLayoutPayload(ctx.deckRoot, rel, lang, labelsFromHtml);
      const blocks =
        body.blocks && Object.keys(body.blocks).length > 0 ? body.blocks : payload.blocks;
  ...
  if (pathname === '/api/slide' && method === 'GET') {
    ...
    try {
      ...
      const layoutApply =
        searchParams.get('layoutApply') === '1' || layoutEdit || layoutProbe;
      const { resolveSlideHtml } = await import('./slideRender.js');
      const html = await resolveSlideHtml(ctx.deckRoot, rel, lang, {
        layoutProbe,
        layoutApply,
  ...
  if (pathname === '/api/admin/users' && method === 'GET') {
    if (!requireAdmin(req, res, searchParams)) return true;
    try {
  ...
  if (pathname === '/api/admin/users' && method === 'POST') {
    ...
    try {
      ...
      const status =
        msg === 'email exists' || msg === 'invalid email' || msg === 'invalid secret' ? 400 : 500;
  ...
  if (pathname.startsWith('/api/admin/users/') && method === 'PATCH') {
    ...
    try {
      ...
      const body = (await readJsonBody(req)) as { secret?: string };
      const secret = String(body.secret ?? '');
  ...
  if (pathname === '/api/comments' && method === 'POST') {
    try {
      const body = (await readJsonBody(req)) as { slideRel?: string; body?: string };
